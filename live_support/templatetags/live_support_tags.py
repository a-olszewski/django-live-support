...
def chat_iframe(context, support_group_id=None):
    ...
    if request.session.get('chat_hash_key'):
        ...
        if chat and not chat[0].ended:
            ...
            iframe_url = reverse('client_chat', args=[chat[0].hash_key])

    return {
        'STATIC_URL': settings.STATIC_URL,
        'url': iframe_url,
        'admin_active': cache.get(cache_key, settings.DEBUG),
        'request': request,
    }

register.inclusion_tag(
    'live_support/chat_iframe.html', takes_context=True)(chat_iframe)
